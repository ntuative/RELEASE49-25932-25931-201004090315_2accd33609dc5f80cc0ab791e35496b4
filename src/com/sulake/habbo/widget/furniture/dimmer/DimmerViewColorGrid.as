package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_466:BitmapData;
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         for each(_loc2_ in colors)
         {
            ...
            _loc3_.width = var_466.width;
            _loc3_.height = var_466.height;
            var_384.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(var_466.width,var_466.height,true,0);
               _loc4_.bitmap.copyPixels(var_466,var_466.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(var_785.width,var_785.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(var_651.width,var_651.height,true,16777215);
               _loc6_.bitmap.copyPixels(var_651,var_651.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = var_384.getGridItemIndex(param1.target as IWindow);
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= var_384.numGridItems)
         {
            return;
         }
         select(var_384.getGridItemAt(param1) as IWindowContainer);
      }
