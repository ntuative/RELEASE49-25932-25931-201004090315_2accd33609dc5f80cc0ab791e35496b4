package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      private static const const_1428:String = "no_reqs_info";
       
      
      private var var_1503:IContainerButtonWindow;
      
      private var _friendList:HabboFriendList;
      
      private var var_1504:IContainerButtonWindow;
      ...
      private function onAcceptAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_friendList.categories.method_3(false) + _friendList.friendRequests.requests.length > _friendList.friendRequests.limit)
         {
         ...
         {
            if(_loc4_.state != FriendRequest.const_314 && _loc4_.state != FriendRequest.const_279)
            {
      ...
      {
         var_1503 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         var_1504 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         var_1504.procedure = onDismissAllButtonClick;
         var_1503.procedure = onAcceptAllButtonClick;
         refreshButtons();
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         for each(_loc4_ in _friendList.friendRequests.requests)
         {
            if(_loc4_.state != FriendRequest.const_314 && _loc4_.state != FriendRequest.const_279)
            {
               _loc4_.state = FriendRequest.const_279;
               refreshRequestEntry(_loc4_);
               if(_friendList && false)
               {
      ...
      public function refreshRequestEntry(param1:FriendRequest) : void
      {
         ...
         else if(param1.state == FriendRequest.const_279)
         {
            _friendList.refreshText(_loc2_,"info_text",true,"${friendlist.request.declined}");
         }
         else if(param1.state == FriendRequest.const_899)
         {
      ...
      private function refreshButtons() : void
      {
         ...
         setEnabled(var_1503,_loc1_);
         setEnabled(var_1504,_loc1_);
      }
      ...
      public function acceptRequest(param1:int) : void
      {
         ...
         if(_friendList.categories.method_3(false) >= _friendList.friendRequests.limit)
         {
            _friendList.showLimitReachedAlert();
            return;
         }
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         _loc2_.state = FriendRequest.const_279;
         var _loc3_:DeclineBuddyMessageComposer = new DeclineBuddyMessageComposer();
