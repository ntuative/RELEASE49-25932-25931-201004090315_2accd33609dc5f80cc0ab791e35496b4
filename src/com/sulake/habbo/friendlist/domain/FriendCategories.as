package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_422:IFriendCategoriesDeps;
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         var _loc5_:* = null;
         flushReceivedStatus();
         findCategory(FriendCategory.const_417).received = true;
         findCategory(FriendCategory.const_875).received = true;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc2_.updatedFriends)
         {
            ...
            var_422.messenger.setFollowingAllowed(_loc4_.id,_loc4_.followingAllowed && _loc4_.online);
            _loc6_ = isFriendOnline(_loc4_.id);
            if(_loc6_ && !_loc4_.online)
            {
               var_422.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_422.notifications.addOfflineNotification(_loc4_.name,_loc4_.realName);
            }
            if(!_loc6_ && _loc4_.online)
            {
               var_422.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_422.view.setNewMessageArrived();
               var_422.notifications.addOnlineNotification(_loc4_.name,_loc4_.figure,_loc4_.realName);
            }
      ...
      public function method_3(param1:Boolean, param2:Boolean = false) : int
      {
         var _loc4_:* = null;
         var _loc3_:int = 0;
         for each(_loc4_ in var_85)
         {
            _loc3_ += _loc4_.method_3(param1,param2);
         }
         return _loc3_;
      }
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_417);
         var _loc3_:FriendCategory = findCategory(_loc2_);
         if(_loc3_ != null)
         {
            _loc3_.addFriend(param1);
            var_845[param1.id] = param1;
            return _loc3_;
         }
