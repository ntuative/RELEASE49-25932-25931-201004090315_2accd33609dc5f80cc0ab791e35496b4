package com.sulake.core.assets
{
   ...
   public class AssetLibraryCollection extends EventDispatcher implements IAssetLibrary
   {
      ...
      public function AssetLibraryCollection(param1:String)
      {
         ...
         var_29 = new Array();
         var_547 = new Array();
         var_1105 = new LoaderContext(false,ApplicationDomain.currentDomain,null);
         var_375 = new AssetLibrary("lib-" + var_643++);
         var_29.push(var_375);
      }
      ...
      private function getNumAssets() : uint
      {
         ...
         while(_loc3_ < _loc2_)
         {
            _loc1_ += IAssetLibrary(var_29[_loc3_]).numAssets;
            _loc3_++;
         }
         return _loc1_;
      }
      ...
      public function unload() : void
      {
         while(false)
         {
            (var_547.pop() as IAssetLibrary).dispose();
         }
         while(false)
         {
            (var_29.pop() as IAssetLibrary).dispose();
         }
      }
      ...
      public function getAssetByIndex(param1:uint) : IAsset
      {
         ...
         while(_loc5_ < _loc4_)
         {
            _loc2_ += IAssetLibrary(var_29[_loc5_]).numAssets;
            if(_loc2_ <= param1)
            {
               return IAssetLibrary(var_29[_loc5_]).getAssetByIndex(param1 - _loc3_);
            }
            _loc3_ = uint(_loc2_);
            _loc5_++;
         }
      ...
      public function loadFromFile(param1:URLRequest, param2:Boolean = false, param3:LoaderContext = null) : LibraryLoader
      {
         ...
         var _loc4_:IAssetLibrary = new AssetLibrary("lib-" + var_643++);
         var_547.push(_loc4_);
      ...
      private function loadEventHandler(param1:LibraryLoaderEvent) : void
      {
         ...
         if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE)
         {
            ...
            _loc4_ = 0;
            while(_loc4_ < var_547.length)
            {
               _loc3_ = var_547[_loc4_] as IAssetLibrary;
               if(_loc3_.url == _loc2_.url)
               {
                  var_547.splice(_loc4_,1);
                  break;
               }
               _loc4_++;
            }
      ...
      private function getAssetNameArray() : Array
      {
         ...
         while(_loc3_ < _loc2_)
         {
            _loc1_ = _loc1_.concat(IAssetLibrary(var_29[_loc3_]).nameArray);
            _loc3_++;
         }
         return _loc1_;
      }
